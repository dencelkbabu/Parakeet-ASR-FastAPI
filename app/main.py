...
def run_asr_on_tensor_chunk(
    ...
    Args:
        audio_chunk_tensor: A 1D PyTorch tensor of the audio chunk.
                            ...
                            AND on the same device as the asr_model.
    ...
    try:
        ...
        try:
            ...
            if websocket.application_state == WebSocketState.CONNECTED:
                ...
                logger.info(f"WebSocket (/ws): Transcription completed in {transcription_duration:.2f} seconds")
...
if __name__ == "__main__":
    ...
    if not asr_model:
        ...
        logger.info(f"Starting server on port {PORT}. Model Rate: {MODEL_SAMPLE_RATE}. Device: {device}")
